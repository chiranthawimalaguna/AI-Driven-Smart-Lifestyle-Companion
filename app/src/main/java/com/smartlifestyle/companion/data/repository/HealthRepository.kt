...
class HealthRepository(
        ...
        val reading = if (fromWatch != null) {
            ...
            val sensed = if (sensorManager.hasStepSensor) {
                withTimeoutOrNull(SENSOR_READ_TIMEOUT_MS) {
                    sensorManager.observeStepCount().firstOrNull()
                }
            } else null
            if (sensed != null) {
                SensorReading(sensed.toFloat())
            } else {
                // Use the latest manually entered value instead of resetting to 0.
                val latestManual = healthDao.observeMetric("steps")
                    .firstOrNull()
                    ?.firstOrNull()

                if (latestManual != null) {
                    ManualReading(latestManual.value)
                } else {
                    ManualReading(0f)
                }
            }
    ...
    suspend fun refreshAmbientLight(): Float? {
        ...
        val lux = withTimeoutOrNull(SENSOR_READ_TIMEOUT_MS) {
            sensorManager.observeAmbientLight().firstOrNull()
        } ?: return null
